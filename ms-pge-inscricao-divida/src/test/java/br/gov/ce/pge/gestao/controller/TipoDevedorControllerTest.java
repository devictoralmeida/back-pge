...
class TipoDevedorControllerTest {
  MockMvc mockMvc;

  @Mock
  private TipoDevedorService service;

  @InjectMocks
  private TipoDevedorController controller;
  ...
  void test_list_all_ok() throws Exception {
    when(service.findAll()).thenReturn(Arrays.asList(TipoDevedorTest.getTipoDevedorPrincipal()));

    mockMvc.perform(MockMvcRequestBuilders.get("/tipo-devedor")
