...
class StatusDebitoControllerTest {
  MockMvc mockMvc;

  @Mock
  private StatusDebitoService service;

  @InjectMocks
  private StatusDebitoController controller;
  ...
  void test_list_all_ok() throws Exception {
    when(service.findAll()).thenReturn(Arrays.asList(StatusDebitoTest.getStatusDebito()));

    mockMvc.perform(MockMvcRequestBuilders.get("/status-debito")
