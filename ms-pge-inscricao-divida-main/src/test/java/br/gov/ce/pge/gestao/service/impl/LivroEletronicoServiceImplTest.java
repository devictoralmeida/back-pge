...
class LivroEletronicoServiceImplTest {
  ...
  public void setUp() {
    ...
    livroEletronicoService = new LivroEletronicoServiceImpl(livroEletronicoRepository, registroLivroRepository, livroEletronicoDao, origemDebitoService);
  }

  @Test
  void abertura_livro_ok() {

    when(livroEletronicoRepository.findBySituacao(SituacaoLivro.ABERTO)).thenReturn(null);
  ...
  void abertura_livro_excecao() {
    ...
    when(livroEletronicoRepository.findBySituacao(SituacaoLivro.ABERTO)).thenReturn(new LivroEletronico());
  ...
  void fechamento_livro_ok() {
    ...
    when(livroEletronicoRepository.findBySituacao(SituacaoLivro.ABERTO)).thenReturn(livroAberto);
  ...
  void find_by_id_model_erro() {
    ...
    Exception erro = Assertions.assertThrows(NegocioException.class, () -> {
      livroEletronicoService.findByIdModel(any());
    });

    assertEquals(MessageCommonsContanst.MSG_ERRO_LIVRO_INEXISTENTE, erro.getMessage());
