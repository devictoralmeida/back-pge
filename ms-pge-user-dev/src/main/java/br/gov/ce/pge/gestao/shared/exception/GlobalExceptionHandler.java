	...
	public ResponseEntity<?> handlePSQLException(DataIntegrityViolationException ex, WebRequest request) {
	 ...
	 private String extractFieldName(String message) {
        ...
        if (message != null && (message.contains(CPF))) {
        	...
        	return "Não foi possível realizar a exclusão! A Permissão está sendo utilizada para um Módulo cadastrado.";
        } else if(message != null && message.contains("fk_tbsistemamodulo_tbmodulo")) {
        	return "Não foi possível realizar a exclusão! O Módulo está sendo utilizado para um Sistema cadastrado.";
        } else if(message != null && message.contains("fk_tbperfilacessosistema_tbsistema")) {
        	return "Não foi possível realizar a exclusão! O Sistema está sendo utilizado para um Perfil de Acesso cadastrado.";
        }
        return null;
	 }
