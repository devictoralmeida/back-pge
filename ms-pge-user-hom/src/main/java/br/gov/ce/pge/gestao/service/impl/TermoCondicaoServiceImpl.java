...
public class TermoCondicaoServiceImpl implements TermoCondicaoService {
	...
	private static final int VERSAO_SECUNDARIA_INICIAL = 0;
	private static final int VERSAO_SECUNDARIA_MAXIMA = 9;
	private static final String ALTERACAO_NAO_PERMITIDA = "Alteração só é permitida na versão vigente!";
	private static final String SEM_HISTORICO = "O termo selecionado não possui histórico de edições.";
	private static final String NAO_ENCONTRADO = "Registro não encontrado.";
	private static final String PORTAL_DIVIDA_ATIVA = "Portal da Dívida Ativa";

	private static final long ITENS_POR_PAGINA = 10;
	private static final long INCREMENTO_PAGINA_PAR = 1;

	private static final String REGEX_SEPARETOR = "\\.";
	private static final String REGEX_VERSAO_TERMO = "%d.%d";
	...
	public void update(UUID id, @Valid TermoCondicaoRequestDto request) {
		...
		if(termoModel.getSistema().getNome().equals(PORTAL_DIVIDA_ATIVA)) {
	...
	private void verificaVersaoVigente(UUID id) {
		...
		if(total > 1) {
			throw new NegocioException(ALTERACAO_NAO_PERMITIDA);
		}
	}

	private String updateVersion(String versao) {
		String[] versionamento = versao.split(REGEX_SEPARETOR);

		int versaoPrincipal = Integer.parseInt(versionamento[0]);
		int versaoSecundaria = Integer.parseInt(versionamento[1]);

		if(versaoSecundaria < VERSAO_SECUNDARIA_MAXIMA) {
			versaoSecundaria++;
		} else {
			versaoPrincipal++;
			versaoSecundaria = VERSAO_SECUNDARIA_INICIAL;
		}

		return String.format(REGEX_VERSAO_TERMO, versaoPrincipal, versaoSecundaria);
	}
	...
	public List<TermoCondicaoSistemaResponseDto> findBySistema() {
		...
		termos.stream().forEach(termo -> {
			...
			dto = termo.getSistema().getNome().equals(PORTAL_DIVIDA_ATIVA) ? setUsuariosPendentesPortalDivida(termo, dto) : setUsuariosPendentesPortalOrigens(termo, dto);
	...
	public PaginacaoResponseDto<List<HistoricoAtualizacaoResponseDto>> findHistorys(UUID id, Integer page) {
		...
		List<AuditoriaDto> historysUpdates =  this.dao.findHistorysUpdates(id, offset, ITENS_POR_PAGINA + INCREMENTO_PAGINA_PAR);
