...
public class UsuarioServiceImpl implements UsuarioService {
	...
	private static final String TEMPLATE_CADASTRO_SENHA_INTERNO_FTLH = "cadastro-senha-interno.ftlh";
	private static final String TEMPLATE_CADASTRO_SENHA_EXTERNO_FTLH = "cadastro-senha-externo.ftlh";
	private static final String MSG_SISTEMA_NAO_RELACIONADO_PERFIL = " não faz parte dos Perfis Selecionados";
	private static final String PGE = "Procuradoria-Geral do Estado do Ceará";
	...
	private void tratarEmail(Usuario usuario) {
		usuario.getSistemas().stream().forEach(sistema -> {
			var template = TEMPLATE_CADASTRO_SENHA_EXTERNO_FTLH;
			...
			if(usuario.getTipoUsuario().equals(TipoUsuario.INTERNO)) {
				template = TEMPLATE_CADASTRO_SENHA_INTERNO_FTLH;
				url = "";
			}
	...
	private Map<String, Object> getParams(Usuario usuario, Sistema sistema, String url) {
		...
		if(sistema.getNome().equals(PORTAL_DIVIDA_ATIVA)) {
			urlAmbiente = this.urlPortalDivida;
		}else if(sistema.getNome().equals(PORTAL_DAS_ORIGENS)){
	...
	private void verificaOrgaoParaUsuarioInterno(TipoUsuario tipoUsuario, String orgao) {
		if(tipoUsuario.equals(TipoUsuario.INTERNO) && !orgao.equals(PGE))
	...
	private void verificaSePerfilAcessoSelecionadoContemNoSistemaSelecionado(List<PerfilAcesso> listaPerfilAcesso, List<Sistema> listaSistema) {
		for (Sistema sistema : listaSistema) {
			...
			if (!sistemaEncontrado)
				throw new NegocioException(sistema.getNome() + MSG_SISTEMA_NAO_RELACIONADO_PERFIL);
		}
	}

	private void verificaUsuarioRedeInterna(UsuarioRequestDto request) {

		if(request.getUsuarioRede() != null && request.getTipoUsuario().equals(TipoUsuario.INTERNO)) {
