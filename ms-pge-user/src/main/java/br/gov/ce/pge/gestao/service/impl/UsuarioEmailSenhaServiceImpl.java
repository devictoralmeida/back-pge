...
public class UsuarioEmailSenhaServiceImpl implements UsuarioEmailSenhaService {
    ...
    private final RequisicaoRecuperarSenhaService recuperacaoService;

    private final UsuarioRepository repository;

    private final CondicaoAcessoRepository condicaoAcessoRepository;

    public UsuarioEmailSenhaServiceImpl(UsuarioConsultaService consultaService, CodigoVerificacaoService codigoService,
                                        EnviarEmailService enviarEmailService, RequisicaoRecuperarSenhaService recuperacaoService,
                                        UsuarioRepository repository, CondicaoAcessoRepository condicaoAcessoRepository) {
        this.consultaService = consultaService;
        this.codigoService = codigoService;
        this.enviarEmailService = enviarEmailService;
        this.recuperacaoService = recuperacaoService;
        this.repository = repository;
        this.condicaoAcessoRepository = condicaoAcessoRepository;
    }
    ...
    private void validaSenhasCadastradas(List<String> list) {
        ...
        if(!condicaoAcesso.isEmpty()) {
            totalSenhasCadastradas = condicaoAcesso.get(SharedConstant.INDICE_INICIAL).getSenhasCadastradas();
        }

        if(!condicaoAcesso.isEmpty() && totalSenhasCadastradas == list.size()) {
            list.remove(SharedConstant.INDICE_INICIAL);
        }
    }
